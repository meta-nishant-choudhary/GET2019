...
<body>
	<center>
		<pre>
			...
			Password	    :	<mvc:input path="password" type ="password" name="password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" oninvalid="this.setCustomValidity('Password should contain upper & lower case and numberic and symbol')" oninput="this.setCustomValidity('')" required="required"/>
			Confirm Password:	<mvc:input path="cPassword" type ="password" name="cpassword" required="required"/>
			Contact No.	    :	<mvc:input path="contact" type ="text" pattern="[0-9]*" maxlength="10" mnlength="1" name="contact" required="required"/>
