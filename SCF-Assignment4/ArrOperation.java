...
public class ArrOperation {
	public static void main(String[] args) {
		int[]mirrorArray = {1, 2, 3, 4, 1, 2, 1, 9};
		System.out.println("max : " + longestMirror(mirrorArray));
	}
	/**
	...
	public static int longestMirror(int[] mirrorArray) {
		...
		for (int loop1 = 0; loop1 < mirrorArray.length - 1; loop1++) {
			int count = 0;
			for (int loop2 = mirrorArray.length - 1; (loop2 > -1) && ((loop1 + count) < mirrorArray.length); loop2--) {
